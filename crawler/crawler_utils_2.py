...
def crawl_all_new_ligues():
    pays = get_data_json("nouveaux_pays")
    ligues = get_data_json("nouvelles_ligues")
    CHROME_DRIVER_PATH = get_os_chromedriver_path()
    chrome_options = Options()
    chrome_options.add_argument("--headless")
    if platform.system() == 'Darwin':
...
def crawl_all_teams(years_limit):
...
def crawl_all_new_teams(years_limit):
    pays = get_data_json("nouveaux_pays")
    ligues = get_data_json("nouvelles_ligues")
    CHROME_DRIVER_PATH = get_os_chromedriver_path()
    chrome_options = Options()
...
def crawl_all_new_ligues_matchs(years_limit, first_ligue_id):
    pays = get_data_json("nouveaux_pays")
    ligues = get_data_json("nouvelles_ligues")
    chrome_options = Options()
    chrome_options.add_argument("--headless")
    ...
    ligues_id = database_fetchall(cursor, "SELECT ID FROM ligues")
    for i in range(0, len(ligues_id)):
        crawl_ligue(driver, subdriver, db, s_db, pays[i], ligues[i], ligues_id[i + first_ligue_id - 1], years_limit)
        db.commit()
        cursor.close()
        db.close()
        db = connect_to_database(s_db)
            ...
            if 'event__round' not in subdiv.get_attribute('class') and 'event__header' not in subdiv.get_attribute('class'):
                try:
                    ...
                    for m in range(0, 2):
                        ...
                        team_id =  database_fetchone(cursor, "SELECT ID FROM teams WHERE TEAM_NAME = '%s' AND LIGUE_ID = %s" % (process_data(team_name), ligue_id))
...
def __main__():
    ...
    #crawl_all_teams(years_limit)
    crawl_all_new_ligues_matchs(years_limit, 65)
